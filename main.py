...
def main():
    ...
    while True:
        ...
        door_location = random.randint(3,res[0]//32 -11)
        door_open = False
        ...
        def open_door(door_open):
            reach = 100
            if not door_open and len(enemies) == 0 and (player.pos[0] - door_pos[0])**2 + (player.pos[1] - door_pos[1])**2 < reach**2:
                door_open = True
                props.remove(door)
            return door_open
        ...
        while not room_completed:
            ...
            if door_open and player.pos[1] < 50:
                room_completed = True
